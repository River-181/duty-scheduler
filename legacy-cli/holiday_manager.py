...
class HolidayTab(ttk.Frame):
    def __init__(self, master):
        super().__init__(master)
        self.holiday_data = load_holidays()
        ...
        btn_frame = ttk.Frame(self)
        btn_frame.pack(pady=5)
        ttk.Button(btn_frame, text="추가", command=self.add_holiday).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="삭제", command=self.delete_holiday).pack(side='left', padx=5)

        self.refresh()

    def refresh(self):
        self.tree.delete(*self.tree.get_children())
        for date, desc in self.holiday_data:
            self.tree.insert('', 'end', values=(date, desc))
    ...
    def add_holiday(self):
        ...
        def save():
            ...
            try:
                ...
                if desc:
                    self.holiday_data.append([date_str, desc])
                    save_holidays(self.holiday_data)
                    self.refresh()
                    popup.destroy()
                else:
                    messagebox.showwarning("입력 오류", "설명을 입력하세요")
            except ValueError:
    ...
    def delete_holiday(self):
        ...
        for item in selected:
            values = self.tree.item(item, 'values')
            self.holiday_data = [row for row in self.holiday_data if row != list(values)]
        save_holidays(self.holiday_data)
        self.refresh()
